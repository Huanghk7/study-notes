...
我们都知道，主从服务器间保持着长连接，并且主服务器收到写操作命令后，会进行命令传播，同步写操作命令到从服务器，但是如果主服务器产生故障，导致从服务器联系不上主服务器，从服务器会进行选举产生新的服务器。如果没有哨兵，则需要我们自己手动的去选定新的主服务器，并通知上游连接到该主服务器的客户端修改地址，这样的操作效率低下。所以在**Redis2.8**之后提供了哨兵机制，它的作用是实现主从节点的故障转移。它会监测主节点的存活，如果发现主节点挂了，它会选举从节点切换为主节点，并且把新的节点信息通知给从节点和客户端。

### 4.2 哨兵机制是怎么工作的

哨兵机制是运行在Redis特殊模式下的一个进程，所以它也是一个节点，这个节点不同主从节点，它是一个观察节点。它主要做三件事情：监控，选主，通知

### 4.3 如何判断主节点故障

哨兵每隔1s就会给主从节点发送**ping**命令，如果主节点没有在规定时间（可以通过配置修改）内响应，哨兵就会判定该主节点主观下线。有主观下线就有客观下线，一个哨兵节点容易造成误判，所以建议搭建哨兵集群（最少需要3台机器部署哨兵集群），通过多个哨兵节点判断，可以减少误判情况。当一个哨兵节点判断主节点【主观下线】后，会向其它节点发起投票，当赞成票数达到规定值则判断为【主观下线】，否则为【客观下线】，即系统或网络故障。
